...
async def update_layered_context(match_id: str, db: "Database", model: str = "gemini-2.5-flash") -> None:
    ...
    4. Persist to cold_summaries and world_state tables via the writer queue.
    """
    try:
        ...
        # Persist summary via writer queue (auto-commits)
...
async def generate_entity_snapshot(
    ...
    try:
        ...
        try:
            ...
            logger.error("Entity snapshot JSON parse failed for %s: %s", match_id, exc)
            return None
