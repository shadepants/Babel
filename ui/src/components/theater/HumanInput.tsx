...
interface HumanInputProps {
  ...
  /** True when the human player went AFK and the engine timed out their turn. */
 ...
 *
 * When humanTimedOut is true, shows an AFK timeout notice instead of
 * the normal awaiting prompt.
 */
export function HumanInput({ matchId, isEnabled, speaker = 'Player', actionMenu, humanTimedOut = false }: HumanInputProps) {
    ...
    <div
      ...
      style={{
        ...
        background: isEnabled
          ...
          : humanTimedOut
            ? 'linear-gradient(180deg, rgba(239,68,68,0.04) 0%, rgba(10,15,30,0.9) 100%)'
            : 'rgba(10,15,30,0.6)',
      }}
    >
      {/* AFK timeout notice */}
      {humanTimedOut && !isEnabled && (
        <div className="text-red-400/70 text-xs font-mono mb-2">
          // AFK timeout &mdash; session continued without your input
        </div>
      )}
